...
describe("ship position validator", () => {
  ...
  test.skip("gather coord location data", () => {
  ...
  test.skip("get column coords for checking", () => {
  ...
  test.skip("get row coords for checking", () => {
  ...
  test.skip("check if position is valid", () => {
  ...
  test.skip("convert coord to matrix table usable", () => {
    expect(convertCoordToMatrix(49)).toEqual([4, 9]);
  });

  test.skip("get the ship direction based on position", () => {
    expect(getShipDirection([4, 5, 6])).toBe("right");
    expect(getShipDirection([4, 14, 24])).not.toBe("right");
  });
});

// test.skip("return the coordinates adjacent to ship from above", () => {
